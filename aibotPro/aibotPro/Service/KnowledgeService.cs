...
namespace aibotPro.Service
{
    public class KnowledgeService : IKnowledgeService
    {
        ...
        public List<Knowledge> GetKnowledgeFiles(int page, int pageSize, string name, out int total, string account = "", string typeCode = "")
        {
        ...
        public async Task<List<string>> CutFile(string content, string processType, string account, string aiModel, string fileCode)
        {
            try
            {
                ...
                else if (processType == "QA")//QA清洗
                {
                    ...
                    chunkList = SplitIntoBlocks(content, 2000, 200);//切片
                    chunkList.RemoveAll(s => string.IsNullOrWhiteSpace(s));//去除空行
                    List<List<QA>> qaList = await StringToQA(aiModel, chunkList, account, QAurl, EmbeddingsApiKey, fileCode);
                    //移除qaList中的null
                    qaList.RemoveAll(s => s == null);
                    //转切片
                    List<string> qaChunkList = new List<string>();
        ...
        public async Task<List<Dictionary<List<float>, string>>> StringToVectorByMilvusAsync(string model, List<string> inputs, string Account, string embeddingsUrl, string embeddingsapikey, string fileCode)
        {
        ...
        private async Task<List<List<QA>>> StringToQA(string model, List<string> inputs, string Account, string embeddingsUrl, string embeddingsapikey, string fileCode)
        {
            var client = new RestClient(embeddingsUrl);
            int EmbRate = 1;//并发
            var semaphore = new SemaphoreSlim(EmbRate); //并发限制
        ...
        public List<KnowledgeType> GetKnowledgeType(int page, int pageSize, string name, out int total, string account = "")
        {
            // 利用IQueryable延迟执行，直到真正需要数据的时候才去数据库查询
