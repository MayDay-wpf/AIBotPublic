  ...
  if (sysmodel == "chat") {
    ...
    try {
      ...
      $("#Q").on("paste", function (event) {
        for (
          var i = 0;
          i < event.originalEvent.clipboardData.items.length;
          i++
        ) {
        ...
        .on("keypress", function (e) {
          ...
          if (enterToSend) {
            ...
            if (
              ...
              // 发送
              e.preventDefault();
              e.stopPropagation(); // 阻止事件冒泡到 document 级别
              sendMsgByCanver();
            }
          } else {
  ...
  if (typeof monaco !== "undefined") {
    ...
    if (!language) {
      monacoEditor.onDidChangeModelContent(() => {
        ...
        // 智能检测规则
        if (
          /^\s*<(!DOCTYPE|html|head|body)/i.test(content) ||
          /<\w+>[\s\S]*<\/\w+>/.test(content)
        ) {
          monaco.editor.setModelLanguage(model, "html");
        } else if (
          /\b(class|function|var|let|const|if|for|while|return|import|export)\b/.test(
            content,
          )
        ) {
          if (
          ...
          /\b(def|class|import|from|if|for|while|return|and|or|not)\b/.test(
            content,
          )
        ) {
          ...
          /\b(int|void|char|double|float|struct|class)\b.*\{/.test(content)
        ) {
          if (
          ...
          /\b(public|private|protected|class|interface|namespace)\b/.test(
            content,
          )
        ) {
          ...
          /^(SELECT|INSERT|UPDATE|DELETE|CREATE|DROP|ALTER)\b/i.test(content)
        ) {
          ...
          /^(\$|>)\s/.test(content) ||
          /\b(apt|yum|brew|chmod|sudo)\b/.test(content)
        ) {
...
function sendMsgByCanver(retryCount = 3) {
  // 在 Coder 模式下，直接获取当前可见的 #Q 输入框的值
  // 因为 chat.js 中的 $Q 变量缓存了原始的 #Q 元素引用
  var msg = $("#Q").val().trim();
  if (msg == "") {
        ...
        $(iframeDoc)
          ...
          .on("click", function (e) {
            if (selectorMode) {
              ...
              const isDuplicate = selectedElementsInfo.some(
                (info) => info.element === this,
              );
