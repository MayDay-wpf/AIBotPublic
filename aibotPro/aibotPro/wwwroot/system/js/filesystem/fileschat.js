...
async function uploadChunk(file, chunk, chunks, start, chunkSize, isManualUpload = false) {
    ...
    if (chunk < chunks) {
        ...
        $.ajax({
            ...
            type: 'POST', data: JSON.stringify({fileName: file.name, totalChunks: chunks}),
...
function getFiles(type) {
    ...
    var data = {
        name: name, page: page_file, pageSize: pageSize_file
    };
    $.ajax({
        type: 'Post',
...
$(document).keypress(function (e) {
    if ($("#searchField").is(":focus")) {
        if (e.which == 13) {
            // 避免回车键换行
            e.preventDefault();
            getFiles('init'); // Go to page 1 on search
        }
    }
});