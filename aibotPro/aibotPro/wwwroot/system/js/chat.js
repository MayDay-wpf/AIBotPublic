...
let shortcuts = true;
let stream = true;
let beforeThink = false;
let beforeThinkModel = "gpt-4.1-nano-openai";
...
let connection = null;
// 发送模式：true = Enter发送, false = Shift+Enter发送，从localStorage读取
let enterToSend = localStorage.getItem("enterToSend") !== "false";
  ...
  if ($("#Q").is(":focus")) {
    ...
    if (enterToSend) {
      ...
      if (
        ...
        (e.which == 10 && e.shiftKey) ||
        (e.which == 13 && e.ctrlKey) ||
        (e.which == 10 && e.ctrlKey)
      ) {
        // 换行
        var input = $("#Q");
      ...
      // 模式二：Shift+Enter发送，Enter换行
      if ((e.which == 13 && e.shiftKey) || (e.which == 10 && e.shiftKey)) {
        // 发送
        e.preventDefault();
        if (sysmodel == "chat") sendMsg();
        else sendMsgByCanver();
...
function updateinputValue(inputId) {
  $(inputId).on("input", function () {
    let value = $(this).val();
    localStorage.setItem(inputId.replace("#", ""), value);
  });
}
...
updateinputValue("#maxTokensValue");
  ...
  if (/image/.test(file.type)) {
    reader.onload = function (event) {
      ...
      var imageFile = dataURLtoFile(
        base64,
        "clipboard_image-" + new Date().toISOString() + ".png",
      );
      var destroyAlert = balert(
        `<i data-feather="loader" style="width:20px;"></i> 正在上传...`,
        "info",
        false,
        0,
        "center",
...
$(function () {
  ...
  if (grouping && !isMobile()) getAIModelListByGroup();
  else getAIModelList();
...
function getAIModelList() {
  // 允许滚动
  $("#AIModel")
    .html(`<div id="searchIcon" style="cursor:pointer;margin:5px;color:gray;">
          ...
          $("#modelList")
            .sortable({
              ...
              start: function (event, ui) {
                // 记录原始顺序
                originalOrder = $("#modelList").sortable("toArray", {
              ...
              stop: function (event, ui) {
                var newOrder = $("#modelList").sortable("toArray", {
                  attribute: "data-model-name",
                });
                // 比较新旧顺序
...
function getAIModelListByGroup() {
  ...
  // 渲染分组列表（会自动调用 checkAndActivateDefaultGroup 来决定默认显示哪个分组）
  getGroupHtml();
  ...
  $.ajax({
    ...
    success: function (res) {
      if (res.success) {
        var data = res.data;
        // 先添加"我的收藏"分组
        groupHtml += `<div class="newgroup-category" data-group="my-collection">
                                <div class="newgroup-category-name">❤️ 我的收藏</div>
                             </div>`;
        // 然后添加"全部"分组
        groupHtml += `<div class="newgroup-category" data-group="">
        ...
        // 检查是否有收藏模型，决定默认激活哪个分组
        checkAndActivateDefaultGroup();
...
// 检查并激活默认分组
function checkAndActivateDefaultGroup() {
  // 检查是否有收藏模型
  $.ajax({
    type: "Post",
    url: "/Users/GetMyCollectionModel",
    dataType: "json",
    headers: {
    ...
    success: function (res) {
      if (res.success && res.data && res.data.length > 0) {
        // 有收藏模型，激活"我的收藏"分组
        $('#AIModel .newgroup-category[data-group="my-collection"]').addClass(
          "active",
        );
        getMyCollectionModels();
      } else {
        // 没有收藏模型，激活"全部"分组
        $('#AIModel .newgroup-category[data-group=""]').addClass("active");
        getModelCard("");
      }
    },
    error: function () {
...
function getModelCard(group) {
  ...
  $.ajax({
    ...
    success: function (res) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        if (model.onceFee > 0) {
          if (topVipType == "VIP|15" || topVipType == "VIP|20") {
            priceHtml = `<span class="badge badge-pill badge-info">输出：${model.vipOnceFee}/次</span>`;
          } else if (topVipType == "VIP|50" || topVipType == "VIP|90") {
            priceHtml = `<span class="badge badge-pill badge-info">输出：${model.svipOnceFee}/次</span>`;
          } else {
            priceHtml = `<span class="badge badge-pill badge-info">输出：${model.onceFee}/次</span>`;
          }
        } else {
          if (topVipType == "VIP|15" || topVipType == "VIP|20") {
            if (model.vipModelPriceInput > 0 && model.vipModelPriceOutput > 0) {
              priceHtml = `<span class="badge badge-pill badge-info">输入：${model.vipModelPriceInput}/1k token</span>
                                       <span class="badge badge-pill badge-info">输出：${model.vipModelPriceOutput}/1k token</span>`;
            } else {
              priceHtml =
            ...
            if (
              model.svipModelPriceInput > 0 &&
              model.svipModelPriceOutput > 0
            ) {
              priceHtml = `<span class="badge badge-pill badge-info">输入：${model.svipModelPriceInput}/1k token</span>
                                       <span class="badge badge-pill badge-info">输出：${model.svipModelPriceOutput}/1k token</span>`;
            } else {
              priceHtml =
                '<span class="badge badge-pill badge-info">免费</span>';
            }
          } else {
            if (model.modelPriceInput > 0 && model.modelPriceOutput > 0) {
              priceHtml = `<span class="badge badge-pill badge-info">输入：${model.modelPriceInput}/1k token</span>
                                       <span class="badge badge-pill badge-info">输出：${model.modelPriceOutput}/1k token</span>`;
            } else {
              priceHtml =
                '<span class="badge badge-pill badge-info">免费</span>';
            }
        ...
        modelCards += `
                    <div class="newgroup-model-card" data-model="${model.modelName}">
                        ...
                        <div class="newgroup-model-price">${priceHtml}</div>
      ...
      bindDefaultModelEvents();
    },
  });
}

// 获取并显示我的收藏模型
function getMyCollectionModels() {
      ...
      if (res.success) {
        ...
        for (var i = 0; i < data.length; i++) {
          ...
          if (model.onceFee > 0) {
            ...
            if (topVipType == "VIP|15" || topVipType == "VIP|20") {
              if (
                model.vipModelPriceInput > 0 &&
                model.vipModelPriceOutput > 0
              ) {
                priceHtml = `<span class="badge badge-pill badge-info">输入：${model.vipModelPriceInput}/1k token</span>
                                           <span class="badge badge-pill badge-info">输出：${model.vipModelPriceOutput}/1k token</span>`;
              } else {
          ...
          modelCards += `
                        <div class="newgroup-model-card collection-model-card" data-model="${model.modelName}">
                            <div class="collection-remove-btn" onclick="removeFromCollection('${model.modelName}')" title="从收藏中移除">
        ...
        // 添加"添加模型"卡片
        modelCards += `
                    <div class="newgroup-model-card add-model-card" onclick="toggleAddModelDropdown(event)">
                        <div class="add-model-icon">
                            <i class="fas fa-plus"></i>
                        </div>
                        <div class="add-model-text">添加模型到收藏</div>
                        <div class="add-model-dropdown" id="addModelDropdown" style="display: none;">
                            <div class="dropdown-header">
                                <input type="text" class="form-control form-control-sm" id="quickModelSearch" placeholder="搜索模型...">
                            </div>
                            <div class="dropdown-body" id="quickModelList">
                                <div class="text-info">加载中...</div>
                            </div>
                        </div>
                    </div>
                    `;
        ...
        $("#AIModel .newgroup-models").html(modelCards);
        bindCollectionModelEvents();
      } else {
        $("#AIModel .newgroup-models").html(
          `<p class="text-danger">加载失败: ${res.msg}</p>`,
        );
      }
    },
    error: function () {
...
// 绑定默认模型事件
function bindDefaultModelEvents() {
  // 展开收起点击事件
...
// 绑定收藏模型事件
function bindCollectionModelEvents() {
  bindDefaultModelEvents(); // 继承默认事件

  // 收藏模型卡片特殊处理 - 阻止移除按钮冒泡
  $(".collection-remove-btn").click(function (e) {
    e.stopPropagation(); // 阻止事件冒泡到卡片点击事件
  });

  // 添加模型卡片特殊处理 - 阻止事件冒泡到模型切换
  $(".add-model-card")
    .off("click")
    .on("click", function (e) {
      e.stopPropagation(); // 阻止事件冒泡
  ...
  $("#quickModelSearch")
    ...
    .on("input", function () {
      const searchText = $(this).val().toLowerCase();
      $(".quick-model-item").each(function () {
        const modelTitle = $(this)
          .find(".quick-model-title")
          .text()
          .toLowerCase();
        const modelName = $(this).data("model-name").toLowerCase();
        if (modelTitle.includes(searchText) || modelName.includes(searchText)) {
          $(this).show();
        } else {
          $(this).hide();
        }
...
function showTooltip(text, e) {
  $("body").append(
    '<div id="customTooltip" style="position: fixed; background: #333; color: #fff; padding: 5px 10px; border-radius: 4px; font-size: 12px; z-index: 9999;">' +
      text +
      "</div>",
  );
  moveTooltip(e);
      ...
      if (res.success) {
        ...
        if (res.length > 0) {
          ...
          if (data.modelPrice.onceFee > 0) {
            ...
            if (topVipType == "VIP|15" || topVipType == "VIP|20") {
              if (
                data.modelPrice.vipModelPriceInput > 0 &&
                data.modelPrice.vipModelPriceOutput > 0
              ) {
              ...
              if (
                data.modelPrice.svipModelPriceInput > 0 &&
                data.modelPrice.svipModelPriceOutput > 0
              ) {
              ...
              if (
                data.modelPrice.modelPriceInput > 0 &&
                data.modelPrice.modelPriceOutput > 0
              ) {
  ...
  if (isMobile()) {
    ...
    if (isShowHistory) {
      ...
      if (sysmodel == "aicanver") {
        chatBody.css("width", "calc(100% - 500px)");
        chatBody.animate(
          {
            marginLeft: "500px",
          },
...
function addLanguageLabels(useSpecificId = false, assistansBoxId = "") {
  var selector =
    useSpecificId && assistansBoxId
      ? $("#" + assistansBoxId + " pre code")
      : $("pre code");
  ...
  selector.each(function () {
    ...
    // 跳过 Mermaid 图表的代码块
    if (
      parentPre.prev(".mermaid").length > 0 ||
      codeBlock.hasClass("language-mermaid")
    ) {
      return;
    }

    // 判断当前代码块是否包含语言标识
    var lang = codeBlock.attr("class")
      ? codeBlock.attr("class").match(/language-(\w+)/)
      : null;
    if (lang) {
      var langLabelContainer = $(
        '<div class="code-lang-label-container"></div>',
      );
      var langLabel = $('<span class="code-lang-label">' + lang[1] + "</span>");
      var toggleBtn = $(
        '<span class="toggle-button"><i class="fas fa-chevron-up"></i> 收起</span>',
      );
      ...
      // 标记代码块已处理过
      parentPre.data("labels-added", true);
    }
  });
}
...
connection.on("ReceiveMessage", function (message) {
  ...
  // 处理MCP工具调用状态
  if (message.tool_call_status) {
    handleToolCallStatus(message);
    return;
  }

  if (!message.isfinish && !processOver) {
    if (jishuqi == 0) {
      ...
      } else if (message.message != null || message.reasoning) {
        if (message.message) sysmsg += message.message;
        if (message.reasoning) thinkContent += message.reasoning;

        if (sysmodel === "chat") {
          stopTimer(`#${assistansBoxId}_timer_first`);
          const chatContentBox = $(`#${assistansBoxId}`);
          ...
          if (hasThinkContent && !thinkBox.length) {
            thinkBox = $(`<details id="${assistansBoxId}-think">
                          <summary>AI 正在思考中(点击展开)...</summary>
                          <div class="think-content"></div>
                          </details>`);
            chatContentBox.before(thinkBox);
          }
          ...
          // 渲染主要内容
          if (sysmsg) {
            chatContentBox.html(md.render(sysmsg));
          }
          // 代码高亮和辅助功能
          $(`#${assistansBoxId} pre code`).each((i, block) =>
          ...
          if (Scrolling == 1) {
            $(".aicanver-chat-container").scrollTop(
              $(".aicanver-chat-container").prop("scrollHeight"),
            );
          }
        }
      }
    ...
    if (sysmodel == "chat") {
      ...
      $('[data-toggle="tooltip"]').tooltip();
      $(`.chat-message[data-group="${chatgroupid}"] .memory`).attr(
        "onclick",
        function () {
      ...
      if (Scrolling == 1) {
        $(".chat-sidebar-body").scrollTop(
          $(".chat-sidebar-body").prop("scrollHeight"),
        );
      }
    }
    feather.replace();
...
function sendMsg(retryCount = 3) {
  var msg = getFullTextContent().trim(); // 使用完整文本内容
  if (msg == "") {
  ...
  var maxtokensText = $("#maxTokensValue").val();
  var maxtokens = parseInt(maxtokensText);
  if (isNaN(maxtokens)) {
    maxtokens = 4095;
  }
  var shortcutSystemPrompt =
  ...
  var data = {
    ...
    beforeThinkModel: beforeThinkModel,
  };
  $("#Q").val("");
  $("#Q").focus();
  var isvip = false;
  ...
  var html = `<div class="chat-message" data-group="${chatgroupid}">
                     <div style="display: flex; align-items: center;">
                        ${vipHead} <div class="fold-chat-group-container">
                                         <i class="fas fa-angle-up fold-chat-group" data-group="${chatgroupid}"></i>
  ...
  if (image_path.length > 0) {
    image_path.forEach(function (path) {
      if (path != "") {
        $("#" + msgid_u).append(
          `<br /><img src="${path.replace("wwwroot", "")}" style="max-width:50%" />`,
        );
      }
    });
  }
    ...
    connection
      ...
      .catch(function (err) {
        ...
        if (retryCount > 0) {
          ...
          $("#" + assistansBoxId).html(
            "发送消息失败,请刷新页面后重试 <a href='javascript:location.reload();'>点击刷新</a>",
          );
          stopTimer(`#${assistansBoxId}_timer_first`);
          stopTimer(`#${assistansBoxId}_timer_alltime`);
          $(".LDI").remove();
...
function getHistoryList(pageIndex, pageSize, reload, loading, searchKey) {
  ...
  if (loading)
    $(".chat-list").append(
      `<li class="divider-text" style="text-align:center;">加载中...</li>`,
    );
  $.ajax({
    type: "Post",
    ...
    success: function (res) {
      ...
      if (res.data.length <= 0) {
        if (pageIndex > 1 && !reload) {
          hasMore = false;
          $(".chat-list").append(
            `<li class="divider-text" style="text-align:center;">没有更多数据了~</li>`,
          );
          //禁用loadMoreBtn
          ...
          //显示没有更多数据了
          html = `<li class="divider-text" style="text-align:center;">
                              <img src = "/system/images/nothing.png" />
                            </li>
                            <li class="divider-text" style="text-align:center;">
                              暂无对话记录,快开始一次对话吧~
                            </li>`;
        }
      }
      for (var i = 0; i < res.data.length; i++) {
        ...
        if (chat.indexOf("aee887ee6d5a79fdcmay451ai8042botf1443c04") != -1) {
          var contentarr = chat.split(
            "aee887ee6d5a79fdcmay451ai8042botf1443c04",
          );
          chat = contentarr[0];
        }
        ...
        var ctrlBtn =
          `<span class="delete-chat">
                                 <i data-feather="x" onclick="deleteChat('` +
          res.data[i].chatId +
          `')"></i>
                               </span>`;
        if (res.data[i].isLock == 1) {
          ctrlBtn =
            `<span class="delete-chat text-success">
                                 <i data-feather="unlock" onclick="unLockChat('` +
            res.data[i].chatId +
            `')"></i>
                               </span>`;
        }
        ...
        if (res.data[i].isTop == 1) {
          //<i data-feather="arrow-up" class="text-success mg-r-10"></i>
  ...
  if (isExpanded) {
    ...
    $.ajax({
      ...
      success: function (res) {
        ...
        if (res.data.length > 0) {
          ...
          $collectionContent.append(
            `<div style="text-align: center; padding: 10px;">该合集下暂无聊天记录</div>`,
          );
        }
      ...
      error: function () {
        ...
        $collectionContent.append(
          `<div style="text-align: center; padding: 10px; color: red;">加载失败，请稍后重试</div>`,
        );
      },
    });
  }
...
$(".chat-sidebar-body").on("scroll", function () {
  if (
    !isLoading &&
    hasMore &&
    $(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight - 20
  ) {
    loadMoreHistory();
  }
});
...
function addChatItemListeners() {
  ...
  $(".delete-chat i")
    ...
    .on("click", function (e) {
      ...
      if (itemtype == "chat") deleteChat($(this).data("chat-id"));
    });
}
...
function multipleChoice() {
  ...
  if (window.isMultipleChoiceMode) {
    ...
    $(".chat-item").each(function () {
      ...
      $(this).prepend(
        '<div class="custom-control custom-checkbox"><input type="checkbox" class="custom-control-input chat-checkbox" id="checkbox-' +
          $(this).attr("id") +
...
function executeBulkAction() {
  ...
  var selectedChats = $(".chat-checkbox:checked")
    .map(function () {
...
function deleteCollection(collectionCode) {
  ...
  showConfirmationModal(
    "提示",
    "确定删除这个合集吗？注意：合集内的对话记录也会一起删除！",
    function () {
      $.ajax({
        type: "Post",
        url: "/Home/DeleteCollection",
        dataType: "json",
        data: {
          collectionCode: collectionCode,
        },
        success: function (res) {
          if (res.success) {
            ...
            const collectionItem = $(
              `li.chat-item[data-chat-id="${collectionCode}"]`,
            );

            // 找到紧随其后的 collection-content 元素
            const contentArea = collectionItem.next("div.collection-content");
...
// 设置发送模式函数
function setSendMode(isEnterToSend) {
  if (isMobile()) return; // 移动端不支持切换

  enterToSend = isEnterToSend;
...
function updateSendModeButton() {
  const buttonIcon = $("#sendModeIcon");
  if (enterToSend) {
    buttonIcon.attr("data-feather", "corner-down-left");
  } else {
    buttonIcon.attr("data-feather", "corner-up-left");
  }
  feather.replace();
}
...
function updatePlaceholderText() {
  ...
  if (enterToSend) {
    textarea.attr(
      "placeholder",
      "输入问题，Enter发送，Shift+Enter换行，首字符输入 / 打开提词本 @ 选择角色",
    );
  } else {
    textarea.attr(
      "placeholder",
      "输入问题，Shift+Enter发送，Enter换行，首字符输入 / 打开提词本 @ 选择角色",
    );
  }
}
...
function deleteChatAll() {
  showPromptModal(
    "提示",
    `请输入<b style="color:red;">“justdoit”</b>以删除全部历史记录<br/>`,
    function (text) {
      if (text == "justdoit") {
        $.ajax({
          type: "Post",
...
// 聊天详情分页相关变量
let chatDetailPageIndex = 1;
let chatDetailPageSize = 10;
let chatDetailIsLoading = false;
let chatDetailHasMore = true;
let currentChatId = "";
...
function showHistoryDetail(id) {
  ...
  // 重置分页变量
  chatDetailPageIndex = 1;
  chatDetailIsLoading = false;
  chatDetailHasMore = true;
  currentChatId = id;
  isInitialLoad = true;
  ...
  $(".chat-item")
    .removeClass("highlight-chat-item")
    .addClass("reset-chat-item");
  $('[id="' + id + '"]').addClass("highlight-chat-item");
  mobileChat(true);

  // 绑定滚动事件监听器
  bindChatDetailScrollListener();

  // 加载第一页数据
  loadChatDetailPage(id, chatDetailPageIndex, true);
}

// 绑定聊天详情滚动监听器
function bindChatDetailScrollListener() {
  // 移除之前的监听器
  chatBody.off("scroll.chatDetail");

  // 添加新的监听器
  chatBody.on("scroll.chatDetail", function () {
    // 检查是否滚动到顶部
    if (
...
// 加载更多聊天详情
function loadMoreChatDetail() {
  if (chatDetailIsLoading || !chatDetailHasMore) return;

  chatDetailPageIndex++;
  loadChatDetailPage(currentChatId, chatDetailPageIndex, false);
}

// 加载聊天详情页面
function loadChatDetailPage(id, pageIndex, isInitial) {
  if (chatDetailIsLoading) return;

  chatDetailIsLoading = true;

  // 记录当前滚动位置
  let currentScrollTop = chatBody.scrollTop();
  let currentScrollHeight = chatBody[0].scrollHeight;

  // 添加加载提示
  if (!isInitial) {
    chatBody.prepend(`<div class="loading-more-history" style="text-align:center;padding:10px;color:#666;">
                            <i class="fas fa-spinner fa-spin"></i> 加载更多历史记录...
                          </div>`);
  }

  $.ajax({
    type: "Post",
    url: "/Home/ShowHistoryDetailPaged",
    dataType: "json",
    data: {
      chatId: id,
      pageIndex: pageIndex,
      pageSize: chatDetailPageSize,
    },
    success: function (res) {
      chatDetailIsLoading = false;
      $(".loading-more-history").remove();

      if (res.success) {
        chatid = id;
        chatDetailHasMore = res.hasMore;

        if (res.data.length === 0) {
          if (isInitial) {
            chatBody.html(
              '<div class="text-center text-muted p-4">暂无对话记录</div>',
            );
          }
          return;
        }

        var html = renderChatHistory(res.data);
        ...
        if (isInitial) {
          ...
          // 计算新的滚动位置，保持用户当前查看的内容不变
          let newScrollHeight = chatBody[0].scrollHeight;
          let scrollDiff = newScrollHeight - currentScrollHeight;
          chatBody.scrollTop(currentScrollTop + scrollDiff);
        }

        // 应用样式和功能
        applyChatHistoryStyles();

        if (!chatDetailHasMore && !isInitial) {
          chatBody.prepend(
            '<div class="text-center text-muted p-2" style="font-size:12px;">已加载全部历史记录</div>',
          );
        }
    ...
    error: function (err) {
      ...
      if (isInitial) {
        ...
        balert("加载更多历史记录失败", "warning", false, 1000, "top");
      }
    },
  });
}
...
function renderChatHistory(data) {
  ...
  for (var i = 0; i < data.length; i++) {
    var content = data[i].chat;
    var msgclass = "chat-message";
    if (data[i].isDel === 2) msgclass = "chat-message chatgroup-masked";

    if (data[i].role == "user") {
      // 检查是否有完整内容（被截取的标记）
      var hasFullContent =
        data[i].chatTitle === "HAS_FULL_CONTENT_CHAT" ||
        data[i].chatTitle === "HAS_FULL_CONTENT_BOTH";
      ...
      if (content.indexOf("aee887ee6d5a79fdcmay451ai8042botf1443c04") == -1) {
        ...
        html += `<div class="${msgclass}" data-group="${data[i].chatGroupId}">
                             <div style="display: flex; align-items: center;">
                               ${vipHead}<div class="fold-chat-group-container">
                                 <i class="fas fa-angle-up fold-chat-group" data-group="${data[i].chatGroupId}"></i>
                                 <span class="fold-text">折叠</span>
                               </div>
                             </div>
                             <div class="chat-message-box">
                               <pre id="${data[i].chatGroupId}" data-chat-code="${data[i].chatCode || data[i].chatGroupId}">${content}</pre>
                               ${
                                 hasFullContent
                                   ? `<div class="expand-content-btn" onclick="expandUserContent('${data[i].chatCode || data[i].chatGroupId}')">
                             ...
                             <div>
                              <i data-feather="refresh-cw" class="chatbtns" onclick="tryAgain('${data[i].chatGroupId}')"></i>
                              <i data-feather="edit-3" class="chatbtns" onclick="editChat('${data[i].chatGroupId}')"></i>
                             </div>
                         </div>`;
      } else {
        ...
        html += `<div class="${msgclass}" data-group="${data[i].chatGroupId}">
                           ...
                           <div class="chat-message-box">
                             <pre id="${data[i].chatCode}" data-chat-code="${data[i].chatCode}">${contentarr[0].replace(/</g, "&lt;").replace(/>/g, "&gt;")}`;
        // 循环添加后续内容
        ...
        html += `</pre>
                               ${
                                 hasFullContent
                                   ? `<div class="expand-content-btn" onclick="expandUserContent('${data[i].chatCode}')">
                                   <i class="fas fa-chevron-down"></i> 展开全文
                               </div>`
                                   : ""
                               }
                           ...
                           <div>
                             <i data-feather="refresh-cw" class="chatbtns" onclick="tryAgain('${data[i].chatCode}')"></i>
                             <i data-feather="edit-3" class="chatbtns" onclick="editChat('${data[i].chatCode}')"></i>
                           </div>
                         </div>`;
        imgBox.push(data[i].chatCode);
      }
    } else {
      var item = {
        id: data[i].chatCode,
      ...
      const thinkContent = data[i].reasoning;

      markdownHis.push(item);
      var markedcontent = md.render(content);
      var firstTime = "";
      var allTime = "";
      if (
        data[i].firstTime != "null" &&
        data[i].allTime != "null" &&
        data[i].firstTime != null &&
        data[i].allTime != null
      ) {
        firstTime = `<span class="badge badge-pill badge-success">${data[i].firstTime}s</span>`;
        allTime = `<span class="badge badge-pill badge-dark">${data[i].allTime}s</span>`;
        if (data[i].firstTime > 10) {
          firstTime = `<span class="badge badge-pill badge-danger">${data[i].firstTime}s</span>`;
        } else if (data[i].firstTime > 5) {
          firstTime = `<span class="badge badge-pill badge-warning">${data[i].firstTime}s</span>`;
        }
      }
      ...
      if (thinkContent) {
        // 检查思考内容是否被截取
        var hasFullThinking =
          data[i].chatTitle === "HAS_FULL_CONTENT_REASONING" ||
          data[i].chatTitle === "HAS_FULL_CONTENT_BOTH";
        thinkBoxHtml = `<details><summary>AI 思考结束（点击展开）</summary>
                                    <div class="think-content" id="think-${data[i].chatCode}">${md.render(thinkContent)}
                                        ${
                                          hasFullThinking
                                            ? `<div class="expand-content-btn" onclick="expandThinking('${data[i].chatCode}')">
                                            <i class="fas fa-brain"></i> 展开完整思考
                                        </div>`
                                            : ""
                                        }
                                    </div>
      ...
      html += `<div class="${msgclass}" data-group="${data[i].chatGroupId}">
                         <div style="display: flex; align-items: center;">
                            <div class="avatar gpt-avatar">${roleAvatar}</div>
                            <div class="nickname" style="font-weight: bold; color: black;">${roleName}</div>
                            <span class="badge badge-info ${data[i].model.replace(".", "")}">${data[i].model}</span>
                            ${firstTime}${allTime}
                         </div>
                         ${thinkBoxHtml}
                        <div class="chat-message-box">
                            <div id="${data[i].chatCode}" data-chat-code="${data[i].chatCode}">${markedcontent}</div>
                            ${
                              hasFullContent
                                ? `<div class="expand-content-btn" onclick="expandAIContent('${data[i].chatCode}')">
                                <i class="fas fa-chevron-down"></i> 展开全文
                            </div>`
                                : ""
                            }
                        </div>
                        <div id="ctrl-${data[i].chatCode}">
                          <i data-feather="copy" class="chatbtns" data-toggle="tooltip" title="复制" onclick="copyAll('${data[i].chatCode}')"></i>
                          <i data-feather="anchor" class="chatbtns" data-toggle="tooltip" title="锚" onclick="quote('${data[i].chatCode}')"></i>
                          <i data-feather="trash-2" class="chatbtns custom-delete-btn-1" data-toggle="tooltip" title="删除" data-chatgroupid="${data[i].chatGroupId}"></i>
                          <i data-feather="cpu" class="chatbtns" data-toggle="tooltip" title="存入记忆" onclick="saveMemory('${data[i].chatGroupId}','${chatid}')"></i>
                          <i data-feather="codepen" class="chatbtns" data-toggle="tooltip" title="显示/隐藏Markdown" onclick="toMarkdown('${data[i].chatCode}')"></i>
                        </div>
                    </div>`;
      hisModelName = data[i].model;
    }
  }

  // 只在初始加载时切换模型
  if (isInitialLoad && autoChange && hisModelName !== thisAiModel) {
    const matchedModel = modelList.find(
      (model) => model.model === hisModelName,
    );
    if (matchedModel) {
...
// 应用聊天历史样式和功能
function applyChatHistoryStyles() {
  ...
  // 处理图片折叠
  $(".chat-message pre[id]").each(function () {
...
// 展开用户消息全文
function expandUserContent(chatCode) {
  var $pre = $(`pre[data-chat-code="${chatCode}"]`);
  var $expandBtn = $pre.siblings(".expand-content-btn");

  $expandBtn.html('<i class="fas fa-spinner fa-spin"></i> 加载中...');

  $.ajax({
    type: "Post",
    url: "/Home/GetFullChatContent",
    dataType: "json",
    data: {
      chatCode: chatCode,
    },
    success: function (res) {
      if (res.success && res.data) {
        var fullContent = res.data.chat;

        // 处理图片内容
        if (
          fullContent.indexOf("aee887ee6d5a79fdcmay451ai8042botf1443c04") != -1
        ) {
          var contentarr = fullContent.split(
            "aee887ee6d5a79fdcmay451ai8042botf1443c04",
          );
          var htmlContent = contentarr[0]
            .replace(/</g, "&lt;")
            .replace(/>/g, "&gt;");
          contentarr.slice(1).forEach((item) => {
            if (item.includes("<img ")) {
              ...
              htmlContent += item.replace(/</g, "&lt;").replace(/>/g, "&gt;");
            }
          });
          $pre.html(htmlContent);
        } else {
          fullContent = fullContent
            .replace(/&lt;/g, "&amp;lt;")
            .replace(/&gt;/g, "&amp;gt;");
          fullContent = fullContent.replace(/</g, "&lt;").replace(/>/g, "&gt;");
          $pre.text(fullContent);
        }

        // 移除展开按钮
        $expandBtn.fadeOut(300, function () {
          $(this).remove();
        });

        // 重新应用图片折叠功能
        if ($pre.find("img").length > 0) {
          initImageFolding(`#${$pre.attr("id")}`);
        }
      } else {
        $expandBtn.html('<i class="fas fa-chevron-down"></i> 展开全文');
        balert("加载失败，请重试", "warning", false, 1000, "top");
      }
    },
    error: function (err) {
      $expandBtn.html('<i class="fas fa-chevron-down"></i> 展开全文');
...
// 展开AI回复全文
function expandAIContent(chatCode) {
  var $div = $(`div[data-chat-code="${chatCode}"]`);
  var $expandBtn = $div.siblings(".expand-content-btn");
  ...
  $.ajax({
    ...
    success: function (res) {
      if (res.success && res.data) {
        var fullContent = res.data.chat;
        var markedcontent = md.render(fullContent);
        $div.html(markedcontent);

        // 更新markdownHis中的内容
        var historyItem = markdownHis.find((item) => item.id === chatCode);
        if (historyItem) {
          historyItem.markdown = fullContent;
        }
        ...
        addLanguageLabels(true, chatCode);
        addCopyBtn(chatCode);
        renderMermaidDiagrams(`#${chatCode}`);
      } else {
        $expandBtn.html('<i class="fas fa-chevron-down"></i> 展开全文');
        balert("加载失败，请重试", "warning", false, 1000, "top");
      }
...
// 展开AI思考全文
function expandThinking(chatCode) {
  var $expandBtn = $(`#think-${chatCode}`).find(".expand-content-btn");
  var $thinkDiv = $(`#think-${chatCode}`);
  ...
  $.ajax({
    ...
    success: function (res) {
      if (res.success && res.data) {
        var fullReasoning = res.data.reasoning;
        var markedReasoning = md.render(fullReasoning);
        $thinkDiv.html(markedReasoning);
        ...
        // 重新应用代码高亮
        $thinkDiv.find("pre code").each(function (i, block) {
          hljs.highlightElement(block);
        });
      } else {
        $expandBtn.html('<i class="fas fa-brain"></i> 展开完整思考');
        balert("加载失败，请重试", "warning", false, 1000, "top");
      }
    },
    error: function (err) {
...
function checkAndShowLoadMoreButton() {
  // 移除现有的加载更多按钮
  $(".load-more-history-btn").remove();

  // 如果没有更多数据或正在加载，不显示按钮
  if (!chatDetailHasMore || chatDetailIsLoading) {
    return;
  }

  // 检查是否有滚动条
  setTimeout(() => {
    const hasScrollbar = chatBody[0].scrollHeight > chatBody[0].clientHeight;

    if (!hasScrollbar) {
      // 没有滚动条，显示加载更多按钮
      const loadMoreBtn = $(`
                <div class="load-more-history-btn" style="text-align:center;padding:15px;border-bottom:1px solid #eee;">
                    <button type="button" class="btn btn-outline-primary btn-sm" onclick="loadMoreChatDetailByButton()">
                        <i class="fas fa-chevron-up"></i> 加载更多历史记录
                    </button>
                </div>
            `);
      chatBody.prepend(loadMoreBtn);
    }
  }, 100);
}

// 通过按钮加载更多聊天详情
function loadMoreChatDetailByButton() {
  $(".load-more-history-btn").remove();
  loadMoreChatDetail();
}
...
function toMarkdown(id) {
  ...
  if (isMobile()) {
    ...
    if (targetElement.getAttribute("data-showing-markdown") === "true") {
      ...
      targetElement.innerHTML = targetElement.getAttribute(
...
function addCopyBtn(id = "") {
  ...
  if (id != "") {
    codebox = $(
      "#" + id + " pre code.hljs, #" + id + ' pre code[class^="language-"]',
    );
  } else {
  ...
  codebox.each(function () {
    ...
    if ($(this).parent().find(".copy-btn").length === 0) {
      if (!isMobile()) copyContainer.append(editBtn); // 先添加编辑按钮
...
function tryAgain(id) {
  ...
  if ($elem.find("img").length > 0) {
    ...
    $elem.find("img").each(function () {
      ...
      if (isURL(imgSrc)) image_path.push(imgSrc);
      else image_path.push("wwwroot" + imgSrc);
      $Q.val($elem.text());
    });
    reviewImg(image_path);
  } else {
...
function saveMemory(chatgroupId, chatId) {
  ...
  $.ajax({
    ...
    success: function (res) {
      loadingOverlay.hide();
      if (res.success) balert("保存成功", "success", false, 1000);
      else balert("保存失败", "danger", false, 1000);
    },
    error: function (err) {
      loadingOverlay.hide();
...
$(document).on("click", ".fold-chat-group-container", function (e) {
  e.stopPropagation();
  const $container = $(this).closest(".fold-chat-group-container");
  const $icon = $container.find(".fold-chat-group");
  const $text = $container.find(".fold-text");
  const groupId = $icon.data("group");
  const $chatGroup = $(`[data-group="${groupId}"]`);

  if ($chatGroup.hasClass("folded")) {
    // 展开对话组
    $chatGroup.removeClass("folded");
    $icon.removeClass("fa-angle-down").addClass("fa-angle-up");
    $text.text("折叠");

    // 显示对话内容，但不影响遮罩层和按钮
    $chatGroup.each(function () {
      $(this).find(".chat-message-box").slideDown(300);
      // 不要影响遮罩层相关元素
      $(this)
        .find("div:last")
        .not(".chatgroup-masked-content, .chatgroup-masked-buttons")
        .slideDown(300);
    });
  } else {
    // 折叠对话组
    ...
    // 隐藏对话内容，但保留遮罩层和按钮
    $chatGroup.each(function () {
      $(this).find(".chat-message-box").slideUp(300);
      // 不要影响遮罩层相关元素
      $(this)
        .find("div:last")
        .not(".chatgroup-masked-content, .chatgroup-masked-buttons")
...
function defaultSeniorSetting() {
  ...
  updateDefaultSettingInput("#maxTokensValue", 4095);
}
...
function updateDefaultSettingInput(inputId, value) {
  $(inputId).val(value);
  localStorage.setItem(inputId.replace("#", ""), value);
}
...
$(document).ready(function () {
  ...
  function addNotificationToList(notice) {
    ...
    const unreadIndicator = notice.isRead
      ? ""
  ...
  $.ajax({
    ...
    success: function (res) {
      ...
      if (res.success) {
        ...
        $("#isvip").show();
        $("#vipendtime").html(
          `<span>${isoStringToDateTime(res.data[0].endTime)}</span> <span class="badge badge-pill badge-warning" style="cursor:pointer" onclick="vipToBalance()">会员时长兑换余额</span>`,
        );
      } else {
        $("#isvip").hide();
        $("#vipendtime").html(
          '<span>未开通</span> <a href="/Pay/VIP" class="text-success">点击前往开通VIP</a>',
        );
      }
    },
  });
      ...
      if (res.success) {
        ...
        $(`.calendar-day`)
          .filter(function () {
            ...
            return (
              dayText === today.toString() &&
              $(this)
                .closest(".calendar")
                .find(".calendar-header")
                .text()
                .includes(`${currentYear}年${currentMonth + 1}月`)
            );
          })
...
function getThisMonthSignInList() {
  $.ajax({
    url: "/Users/GetThisMonthSignInList",
    type: "post",
    dataType: "json",
    success: function (res) {
...
function setNetworkMode(mode) {
  ...
  if (mode === "off") {
    globe = false;
    multimodal = false;
    $globeSpan.removeClass("badge-success").addClass("badge-secondary");
    $globeSpan.html(
      '<i data-feather="globe"></i> 联网 <i data-feather="chevron-up" style="width:14px;height:14px;"></i>',
    );
  } else if (mode === "web") {
    globe = true;
    multimodal = false;
    $globeSpan.removeClass("badge-secondary").addClass("badge-success");
    $globeSpan.html(
      '<i data-feather="globe"></i> 普通联网 <i data-feather="chevron-up" style="width:14px;height:14px;"></i>',
    );
    ...
    $globeSpan.removeClass("badge-secondary").addClass("badge-success");
    $globeSpan.html(
      '<i data-feather="globe"></i> 多模态联网 <i data-feather="chevron-up" style="width:14px;height:14px;"></i>',
    );
  }
...
// 处理MCP工具调用状态
function handleToolCallStatus(message) {
  const {
    tool_call_name,
    tool_call_arguments,
    tool_call_status,
  ...
  // 查找或创建工具调用容器
  let toolCallContainer = $(`#tool-call-${tool_call_id}`);

  if (toolCallContainer.length === 0) {
    // 创建工具调用UI容器
    toolCallContainer = createToolCallUI(
      tool_call_id,
      tool_call_name,
      tool_call_arguments,
    );
    ...
    // 将工具调用UI插入到聊天框中
    const chatContentBox = $(`#${assistansBoxId}`);
    if (chatContentBox.length > 0) {
      chatContentBox.before(toolCallContainer);
    } else {
      // 如果聊天框不存在，插入到消息区域
      $("#chat-body").append(toolCallContainer);
    }
  }

  // 更新工具调用状态
  updateToolCallStatus(toolCallContainer, tool_call_status, tool_call_result);
}

// 创建工具调用UI
function createToolCallUI(toolCallId, toolName, arguments) {
  const toolCallHtml = `
        <div id="tool-call-${toolCallId}" class="tool-call-container mb-2" style="max-width: 320px; border-radius: 10px !important; overflow: hidden;">
            <div class="tool-widget p-3 shadow-sm" style="background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%); border: 1px solid rgba(0,0,0,0.08); border-radius: 10px !important;">
                <div class="d-flex align-items-center mb-2">
                    <div class="tool-icon-wrapper me-2" style="width: 24px; height: 24px; background: rgba(13, 110, 253, 0.1); border-radius: 6px; display: flex; align-items: center; justify-content: center;">
                        <i class="fas fa-magic" style="font-size: 11px; color: #0d6efd;"></i>
                    </div>
                    <span class="tool-name fw-medium" style="font-size: 13px; color: #495057;">${toolName}</span>
                    <div class="ms-auto d-flex align-items-center">
                        <span class="tool-status px-2 py-1 rounded-pill" style="font-size: 10px; background: rgba(108, 117, 125, 0.15); color: #6c757d; font-weight: 500;">执行中</span>
                        <div class="spinner-border text-primary tool-loading ms-2" role="status" style="width: 14px; height: 14px; border-width: 2px;">
                            <span class="sr-only">Loading...</span>
                        </div>
                    </div>
                </div>
                <div class="tool-result" style="display: none;">
                    <div class="result-divider mb-2" style="height: 1px; background: rgba(0,0,0,0.06);"></div>
                    <div class="result-content" style="font-size: 11px; color: #868e96; line-height: 1.4; background: rgba(255,255,255,0.5); padding: 8px 10px; border-radius: 6px; font-family: 'SF Mono', Monaco, 'Cascadia Code', 'Roboto Mono', Consolas, 'Courier New', monospace;"></div>
                </div>
            </div>
        </div>
    `;
...
// 更新工具调用状态
function updateToolCallStatus(container, status, result) {
  const statusBadge = container.find(".tool-status");
  const loadingDiv = container.find(".tool-loading");
  const resultDiv = container.find(".tool-result");
  const resultContent = container.find(".result-content");
  ...
  switch (status) {
    ...
    case "success":
      ...
      if (result && result.trim()) {
        ...
        if (result.length > 80) {
          displayResult = result.substring(0, 77) + "...";
        }
        resultContent.text(displayResult);
        resultDiv.slideDown(200);
      }
    ...
    case "error":
      ...
      if (result) {
        let errorMsg =
          result.length > 60 ? result.substring(0, 57) + "..." : result;
        resultContent.text(errorMsg).css({
          color: "#dc3545",
...
// 从收藏中移除模型
function removeFromCollection(modelName) {
  // 获取当前收藏的所有模型
  $.ajax({
    type: "Post",
    url: "/Users/GetMyCollectionModel",
    ...
    success: function (res) {
      if (res.success) {
        const currentModels = res.data.map((model) => model.modelName);
        const filteredModels = currentModels.filter(
          (name) => name !== modelName,
        );
        const newModelNames = filteredModels.join(",");

        // 更新收藏
        updateCollectionModels(newModelNames, function () {
          balert("已从收藏中移除", "success", false, 1500);
          // 重新加载收藏模型列表
          getMyCollectionModels();
        });
      } else {
        balert("获取收藏列表失败", "danger", false, 1500);
      }
    },
    error: function () {
      balert("获取收藏列表失败", "danger", false, 1500);
...
// 切换添加模型下拉菜单
function toggleAddModelDropdown(event) {
  event.stopPropagation();

  const dropdown = $("#addModelDropdown");
  const isVisible = dropdown.is(":visible");

  // 隐藏所有其他下拉菜单
  $(".add-model-dropdown").hide();

  if (!isVisible) {
    dropdown.show();
    loadQuickModelList();
    $("#quickModelSearch").focus();
  }
}

// 点击其他地方隐藏下拉菜单
$(document).on("click", function (e) {
  if (!$(e.target).closest(".add-model-card").length) {
    $(".add-model-dropdown").hide();
  }
});

// 加载快速添加模型列表
function loadQuickModelList() {
  $("#quickModelList").html('<div class="text-info">加载中...</div>');
    ...
    success: function (res) {
      if (res.success || res.data) {
        ...
        $.ajax({
          ...
          headers: {
            Authorization:
              "Bearer " + localStorage.getItem("aibotpro_userToken"),
          },
          success: function (collectionRes) {
            const collectedModels = collectionRes.success
              ? collectionRes.data.map((model) => model.modelName)
              : [];

            let modelListHtml = "";
            allModels.forEach((model) => {
              const isCollected = collectedModels.includes(model.modelName);
              if (!isCollected) {
                modelListHtml += `
                                    <div class="quick-model-item" data-model-name="${model.modelName}" onclick="quickAddToCollection('${model.modelName}')">
                                        <div class="quick-model-title">${model.modelNick}</div>
                                        <small class="text-muted">${model.modelName}</small>
                                        <i class="fas fa-plus quick-add-icon"></i>
                                    </div>
                                `;
              }
            });
            ...
            $("#quickModelList").html(
              modelListHtml ||
                '<div class="text-info">所有模型都已在收藏中</div>',
            );
          },
          error: function () {
            // 如果获取收藏列表失败，显示所有模型
            ...
            $("#quickModelList").html(modelListHtml);
          },
        });
      } else {
        $("#quickModelList").html(
          '<div class="text-danger">加载模型列表失败</div>',
        );
      }
    },
    error: function () {
...
// 快速添加到收藏
function quickAddToCollection(modelName) {
  // 获取当前收藏的所有模型
  $.ajax({
    type: "Post",
    url: "/Users/GetMyCollectionModel",
    ...
    success: function (res) {
      const currentModels = res.success
        ? res.data.map((model) => model.modelName)
        : [];

      if (!currentModels.includes(modelName)) {
        currentModels.push(modelName);
        const newModelNames = currentModels.join(",");

        // 更新收藏
        updateCollectionModels(newModelNames, function () {
          balert("已添加到收藏", "success", false, 1500);
          // 隐藏下拉菜单
          $(".add-model-dropdown").hide();
          // 如果当前显示的是收藏分组，重新加载
          const activeGroup = $("#AIModel .newgroup-category.active").data(
            "group",
          );
          if (activeGroup === "my-collection") {
            getMyCollectionModels();
          }
        });
      } else {
        balert("该模型已在收藏中", "warning", false, 1500);
      }
    },
    error: function () {
      // 如果获取收藏失败，直接添加
      updateCollectionModels(modelName, function () {
        balert("已添加到收藏", "success", false, 1500);
        $(".add-model-dropdown").hide();
      });
    },
      ...
      if (res.success || res.data) {
        ...
        $.ajax({
          ...
          success: function (collectionRes) {
              ...
              if (!isCollected) {
                ...
                if (model.onceFee > 0) {
                  ...
                  if (topVipType == "VIP|15" || topVipType == "VIP|20") {
                    ...
                    if (
                      model.modelPriceInput > 0 &&
                      model.modelPriceOutput > 0
                    ) {
                      priceHtml = `<span class="badge badge-pill badge-info">输入：${model.modelPriceInput}/1k token</span>
                                                       <span class="badge badge-pill badge-info">输出：${model.modelPriceOutput}/1k token</span>`;
                    } else {
                ...
                modelListHtml += `
                                    <div class="available-model-item" data-model-name="${model.modelName}">
                                        <div class="model-title">${model.modelNick}</div>
                                        <small class="text-muted">${model.modelName}</small>
                                        <div class="model-price">${priceHtml}</div>
                                        <button class="btn btn-sm btn-primary add-to-collection-btn" 
                                                onclick="addToCollection('${model.modelName}')">
            ...
            $("#availableModelsList").html(
              modelListHtml || '<p class="text-info">所有模型都已在收藏中</p>',
            );
          },
          error: function () {
            // 如果获取收藏列表失败，显示所有模型
            ...
            allModels.forEach((model) => {
              var priceHtml =
                model.modelPriceInput > 0
                  ? `输入：${model.modelPriceInput}/1k，输出：${model.modelPriceOutput}/1k`
                  : "免费";
            ...
            $("#availableModelsList").html(modelListHtml);
          },
        });
      } else {
        $("#availableModelsList").html(
          '<p class="text-danger">加载模型列表失败</p>',
        );
      }
    },
    error: function () {
...
// 添加模型到收藏
function addToCollection(modelName) {
  // 获取当前收藏的所有模型
  $.ajax({
    type: "Post",
    url: "/Users/GetMyCollectionModel",
    ...
    success: function (res) {
      ...
      if (!currentModels.includes(modelName)) {
        ...
        updateCollectionModels(newModelNames, function () {
          balert("已添加到收藏", "success", false, 1500);
          // 移除该项或重新加载列表
          $(`.available-model-item[data-model-name="${modelName}"]`).fadeOut();
          // 如果当前显示的是收藏分组，重新加载
          const activeGroup = $("#AIModel .newgroup-category.active").data(
            "group",
          );
...
// 更新收藏模型列表
function updateCollectionModels(modelNames, successCallback) {
  $.ajax({
    type: "Post",
    url: "/Users/UpdateMyCollectionModel",
    dataType: "json",
    ...
    success: function (res) {
      if (res.success) {
        ...
        balert("更新收藏失败: " + res.msg, "danger", false, 2000);
      }
    },
    error: function () {
      balert("更新收藏失败", "danger", false, 2000);
    },
  });
}

// 粘贴功能更新 - 强制缓存刷新 2024-12-20 15:30
