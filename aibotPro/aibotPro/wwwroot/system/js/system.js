...
function price() {
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.success) {
                res = res.data;
                var str = '<div style="max-height:500px;overflow-y: scroll;"><p>1k token≈600汉字<p><p>计费算法=(输入+输出)*b倍率，倍率小于1则是有折扣<p><table><tr><td>模型</td><td>输入价格（1k token）</td><td>输出价格（1k token）</td><td>VIP输入价格（1k token）</td><td>VIP输出价格（1k token）</td><td>普通用户倍率</td><td>VIP倍率</td></tr>';
