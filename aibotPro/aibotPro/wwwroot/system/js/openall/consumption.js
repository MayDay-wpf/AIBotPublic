$(function () {
    ...
    $("#consumption_datastatistics_nav").addClass('active');
});
let page = 1;
let page_size = 15;
let total = 0;
...
function updateLogList(logs) {
    ...
    for (var i = 0; i < logs.length; i++) {
        str += `<tr><td>` + logs[i].account + `</td><td>` + logs[i].modelName + `</td><td>` + logs[i].useMoney + `</td><td>` + logs[i].inputCount + `</td><td>` + logs[i].outputCount + `</td><td>` + logs[i].createTime + `</td></tr>`;
    }
    $logList.html(str);
}
