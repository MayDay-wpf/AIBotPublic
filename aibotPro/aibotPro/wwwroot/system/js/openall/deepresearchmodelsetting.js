$(function () {
    ...
    $("#deepresearchmodel_aisystem_nav").addClass('active');
    getDeepResearchModelSetting();
});
...
function saveDeepResearchModelSetting() {
    var deepResearchModels = [];
    ...
    rows.each(function (index, row) {
        ...
        var adminPrompt = $(row).find("input").eq(8).val();
        var modelInfo = $(row).find("input").eq(9).val();
        if (!removeSpaces(nickname) || !removeSpaces(name) || !removeSpaces(baseUrl) || !removeSpaces(apiKey)) {
            balert('请将空的【自定义编程模型】输入行删除，或填写完整', 'danger', false, 1500, 'top');
            issave = false;
            return;
        } else {
            deepResearchModels.push({
                ModelNick: nickname,
                ModelName: name,
                BaseUrl: baseUrl,
                ApiKey: apiKey,
    ...
    if (issave) {
        ...
        $.ajax({
            type: 'POST',
            url: '/OpenAll/SaveDeepResearchModelSetting',
            dataType: 'json',
            data: {
                deepResearchModels: JSON.stringify(deepResearchModels)
            },
...
function getDeepResearchModelSetting() {
    //发起请求
    $.ajax({
        type: 'Post',
        url: '/OpenAll/GetDeepResearchModelSetting',
        success: function (res) {
            if (res.success) {
                var data = res.data;
                if (data == null)
...
function editRow(row) {
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body" style="max-height: 600px; overflow-y: auto;">
                        <form id="editRowForm">
                            ...
                            <div class="form-group">
                                <label>排序</label>
                                <input type="number" class="form-control" id="edit-seq" value="${currentRow.find('td:eq(6) input').val()}"/>
                            </div>
                            <div class="form-group">
                                <label>流延时(ms)</label>
                                <input type="number" class="form-control" id="edit-delay" value="${currentRow.find('td:eq(7) input').val()}"/>
                            </div>
                            <div class="form-group">
                                <label>最低使用余额</label>
                                <input type="text" class="form-control" id="edit-minimumBalance" value="${currentRow.find('td:eq(8) input').val()}"/>
                            </div>
                            <div class="form-group">
                                <label>预设系统提示词</label>
    ...
    $('#save-row-btn').click(function () {
        ...
        currentRow.find('td:eq(6) input').val($('#edit-seq').val());
        currentRow.find('td:eq(7) input').val($('#edit-delay').val());
        currentRow.find('td:eq(8) input').val($('#edit-minimumBalance').val());
